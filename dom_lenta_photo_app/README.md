...
- `Procfile` — альтернативная команда запуска `web: gunicorn --workers 1 --threads 4 --timeout 600 app:app`;
...
5. Render прочитает `render.yaml`, установит зависимости командой `pip install --upgrade --no-cache-dir -r dom_lenta_photo_app/requirements.txt` и запустит приложение командой `gunicorn --chdir dom_lenta_photo_app --workers 1 --threads 4 --timeout 600 app:app`.
...
3. Укажите параметры:
   ...
   - **Start Command**: `gunicorn --chdir dom_lenta_photo_app --workers 1 --threads 4 --timeout 600 app:app`;
...
## Радикальные варианты для production

Текущая версия остаётся совместимой с Render Web Service, но обработка изображений — тяжёлая CPU/RAM-задача. Для стабильной production-эксплуатации лучше вынести обработку из web-процесса в отдельный worker/очередь:

1. Web Service только принимает файлы и показывает статус.
2. Worker Service обрабатывает rembg и вызывает FLUX.
3. Результаты и ZIP хранить в S3/R2/Cloudinary или Render Disk.
4. Статусы хранить в Redis/Postgres, а не in-memory.

На Render Free текущий компромисс — `gunicorn --workers 1 --threads 4 --timeout 600`, чтобы status polling не падал, пока фоновая задача обрабатывает изображения.
